	...
	if (isset($_POST)) {
		...
		if (count($comment_rows) > 0) {
			...
			foreach ($comment_rows as $row) {			
				...
				if ($saved_id == $row['id']) {
					echo  "<div class='container_message darker'><p>" . $row['text'] . "\n\n<span style='float: right;'>-" . $row['display_name'] . "</span><span class='time-left'>" . date("h:i A", $time) . "</span>" . "</p><button style='font-size:24px; color:red'>Button <i class='fa fa-trash'></i></button></div>";
				}
				else {
