	...
	if (isset($_POST)) {
		...
		if (count($comment_rows) > 0) {
			...
			foreach ($comment_rows as $row) {			
				...
				if ($saved_id == $row['id']) {
					echo  "<div class='container_message darker'><p>" . $row['text'] . "\n\n<span style='float: right;'>-" . $row['display_name'] . "</span><span class='time-left'>" . date("h:i A", $time) . "</span>" . "</p><span style='float: right;'><p class='delComment'><button class='w3-btn w3-red w3-small delBtn'><i class='material-icons'>delete</i></button></p></span></div>"; /*<button class='btnRemove'><i class='fa fa-trash'></i>Trash</button>*/		
